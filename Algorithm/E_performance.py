...
def performance(domain):
    ...
    for topic in topic_list:
        ...
        # 默认实验结果
        with open('../output/' + domain + '/Algorithm/D_analyse/' + topic.strip() + '.txt', 'r',
        ...
        for l in labeled_facet_list:
            ...
            if (flag):
                fn += 1
            flag = True
        p = tp / len(model_facet_list)
        # if 'history' not in labeled_facet_list:
        #     r = tp / (len(labeled_facet_list) + 1)
        # else:
    ...
    with open('../output/' + domain + '/Algorithm/E_performance/result.txt', 'w', encoding='utf-8') as f:
        f.writelines(['P: ', str(P) + '\n', 'R: ', str(R) + '\n', 'F1: ', str(F1) + '\n'])


performance('Data_mining')
